...
def __handle_column_match_with_regex():
    ...
    function_code=["import re", os.linesep, "def column_selector_regex(regex_str, cur_df):", os.linesep]
...
def __handle_column_match_with_template():
    function_code=["import itertools", os.linesep, "from string import Template", os.linesep, "def column_selector_template(template_input, template_string, substitute_string):", os.linesep]
    ...
    function_code.extend(["\t", "for input in input_combinations:", os.linesep])
    function_code.extend(["\t\t", "result.append(template.substitute(substitute_string))", os.linesep])
...
def __generate_pattern_creator_for_select_expr_code():
    ...
    code.extend(['\tif (pop_last):', os.linesep])
...
def __generate_single_select_expr_generator_code():
    ...
    code.extend(['\t\t\tpattern = pattern_creator_for_select_expr(input_cols, output_cols, "", "", ", ", True, "", "")', os.linesep])
    code.extend(['\t\telse:', os.linesep])
    code.extend(['\t\t\tpattern = pattern_creator_for_select_expr(input_cols, output_cols, operation + "(", ")", ", ", True, "", "")', os.linesep])
    code.extend(['\telse:', os.linesep])
    code.extend(['\t\tif (operation == "concat"):', os.linesep])
    code.extend(['\t\t\tpattern = pattern_creator_for_select_expr(input_cols, output_cols, "", "", ", ", True, "concat(", ")")', os.linesep])
    code.extend(['\t\telse:', os.linesep])
    code.extend(['\t\t\tpattern = pattern_creator_for_select_expr(input_cols, output_cols, "", " " + operation, " ", True, "", "")', os.linesep])
