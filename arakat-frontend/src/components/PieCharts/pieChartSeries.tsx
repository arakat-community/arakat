...
export class PieChartSeriesComponent<T> extends PureComponent<IPieChartSeriesProps<T>> {
    ...
    public componentDidMount(): void {
        const { chartSeries, category, is3D, data, fontSize } = this.props.chartConfig;
        ...
        chartSeries.forEach((chartSerie) => {
            ...
            addedSerie.dataFields.category = category.field;

            addedSerie.slices.template.stroke = Am4core.color(chartSerie.style.fill);
            addedSerie.slices.template.strokeWidth = chartSerie.style.strokeWidth;
        });
    }
    /**
